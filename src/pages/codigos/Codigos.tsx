...
export const Codigos = () => {
  ...
  if (loading) return <p className="text-muted text-sm">Carregando dados do GitHub…</p>
  ...
  return (
    <div className="max-w-4xl space-y-12">
      <div>
        ...
        <p className="text-secondary text-sm">Tudo que consumo direto da API do GitHub.</p>
      ...
      {data.org && (
        <section>
          <h2 className="text-xs uppercase tracking-wide text-muted mb-3">Comunidade</h2>
          <div className="flex items-center gap-4 bg-surface rounded-xl p-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted">{data.org.description}</p>
              <a href={data.org.html_url} target="_blank" rel="noopener noreferrer" className="text-xs text-secondary hover:text-primary underline underline-offset-2">
                Ver no GitHub
              </a>
            </div>
          </div>
      ...
      <section>
        <h2 className="text-xs uppercase tracking-wide text-muted mb-3">Repositórios ({data.repos.length})</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {data.repos.map((repo) => (
            <div key={repo.id} className="bg-surface rounded-xl p-4">
              <p className="text-sm font-medium">{repo.name}</p>
              <p className="text-xs text-muted mt-1 mb-3 line-clamp-2">{repo.description}</p>
              <div className="flex flex-wrap gap-3">
                <a href={repo.html_url} target="_blank" rel="noopener noreferrer" className="text-xs text-secondary hover:text-primary underline underline-offset-2">
                  Repositório
                </a>
      ...
      <section>
        <h2 className="text-xs uppercase tracking-wide text-muted mb-3">Em progresso ({data.issues.length})</h2>
        {data.issues.length === 0 ? (
          <p className="text-sm text-muted">Nada em progresso nas Especiais agora.</p>
        ) : (
          <div className="space-y-2">
            {data.issues.map((issue) => (
              <a key={issue.id} href={issue.html_url} target="_blank" rel="noopener noreferrer" className="block bg-surface rounded-lg px-4 py-2 hover:bg-surface-solid transition-colors">
                <p className="text-sm">{issue.title}</p>
                <p className="text-xs text-muted">{issue.repository} · #{issue.number}</p>
              </a>
            ))}
          </div>
        )}
      ...
      <section>
        <h2 className="text-xs uppercase tracking-wide text-muted mb-3">Gists ({data.gists.length})</h2>
        <div className="space-y-2">
          {data.gists.map((gist) => (
            <a key={gist.id} href={gist.html_url} target="_blank" rel="noopener noreferrer" className="block bg-surface rounded-lg px-4 py-2 hover:bg-surface-solid transition-colors">
